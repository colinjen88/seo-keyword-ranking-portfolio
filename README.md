...
- **即時搜尋篩選**：在首頁標題旁新增了放大鏡圖示按鈕，點擊後可向下推開搜尋框，實現關鍵字與統計數據的即時即打即顯（Real-time filtering）。
- **新增 7 項金幣實績**：加入了 2026/03/03 最新的搜尋實績，包含「五大金幣」、「金幣溢價」、「鑄幣溢價」等各項排名前三的熱門關鍵字。
- **數據與排版優化**：修復了大量 Gallery 卡片跑版的問題，並針對統計區塊（Stats Bar）進行了即時篩選的響應優化，讓收錄案例與排名數量能隨搜尋結果動態調整。
